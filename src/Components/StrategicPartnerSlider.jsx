  ...
  return (
    <section className="py-10 bg-gray-50 dark:bg-gray-900/50">
      <div className="wrapper">
        <div
          data-aos="fade-up"
          className="max-w-2xl mx-auto bg-white dark:bg-white/5 rounded-2xl border border-gray-200 dark:border-white/10 p-6 shadow-lg"
        >
          {/* Headline */}
          <div className="mb-6">
            <h2 className="text-lg sm:text-xl font-normal dark:text-gray-300 text-darkbackground mb-1">
              Your Strategic Partner for
            </h2>
            <div className="relative inline-block w-full">
              <h2 className="main-title !text-xl sm:!text-2xl md:!text-3xl !font-bold">
                AI-Driven Digital Transformation and Enterprise Software Engineering
              </h2>
              <div
                className="mt-3 h-1 rounded-full w-full max-w-xs mx-auto"
                style={{
                  background:
                    "linear-gradient(to right, #8b5cf6, #3c81f6, #fb923c)",
                }}
              />
            </div>
          </div>
          ...
          <div ref={sliderRef} className="keen-slider">
            {SLIDES.map((slide) => (
              <div
                key={slide.id}
                className="keen-slider__slide min-h-[150px]"
              >
                <div className="flex flex-col gap-3">
                  <h3 className="text-lg sm:text-xl font-bold dark:text-white text-darkbackground">
                    {slide.title}
                  </h3>
                  <div className="space-y-2">
                    {slide.content.split("\n").map((line, idx) => (
                      <p
                        key={idx}
                        className="desc text-sm leading-relaxed dark:text-gray-300 text-darkbackground/90"
                      >
                        {line.trim()}
                      </p>
                    ))}
          ...
          <div className="flex gap-2 mt-6 justify-center">
            <button
              ...
              className="flex-shrink-0 w-8 h-8 rounded-full p-[1px] bg-gradient-to-r from-violet-500 via-primary to-orange-400 hover:opacity-90 transition-opacity flex items-center justify-center"
            >
              <span className="w-full h-full rounded-full bg-white dark:bg-darkblack flex items-center justify-center">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-4 w-4 dark:text-white text-darkbackground"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
